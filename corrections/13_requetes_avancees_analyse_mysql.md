# Corrections — 13. Requêtes avancées d'analyse (MySQL)
...
WITH month_customer AS (
  SELECT
    DATE_FORMAT(o.ordered_at, '%Y-%m-01') AS mois,
    c.id AS customer_id,
    c.email,
    SUM(oi.quantity * oi.unit_price) AS total_paid
  FROM customers c
  ...
  JOIN order_items oi ON oi.order_id = o.id
  WHERE o.status = 'paid'
  GROUP BY DATE_FORMAT(o.ordered_at, '%Y-%m-01'), c.id, c.email
),
ranked AS (
  SELECT
    mois,
...
WITH order_totals AS (
  ...
  JOIN order_items oi ON oi.order_id = o.id
  WHERE o.status = 'paid'
  GROUP BY DATE_FORMAT(o.ordered_at, '%Y-%m-01'), o.id
),
scored AS (
  SELECT
    mois,
